...
def run_blf_packing(container_data: Dict, items_data: List[Dict], groups_data: List[Dict], constraints: Dict) -> Dict:
    ...
    try:
        ...
        print(f"unplaced items: {[item['id'] for item in unplaced_items]}")
        weight_unpacked = sum(b.weight for b in unpacked)
        volume_unpacked = sum(b.get_volume() for b in unpacked)
        print(f"packed volume: {sum(b.get_volume() for b in packed)}")
        print(f"Total unpacked weight: {weight_unpacked}, unpacked volume: {volume_unpacked}")
